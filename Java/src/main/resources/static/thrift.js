...
Thrift.Protocol.prototype = {
    ...
    rpush: function (obj) {
        ...
        this.rstack.push(obj);
    },
    rpop: function () {
        var obj = this.rstack.pop();
        //console.log("pop: " + JSON.stringify(obj));
        return obj;
    },
    /**
     @class
    ...
    readMessageBegin: function () {
        this.rstack = [];
        this.rpos = [] ; //not used actually but deserilizer expects it -.-
        ...
        var r = {};
        //console.log("Deserialize: " + JSON.stringify(this.robj));
        ...
        //get to the main obj
        this.rpush(this.robj.shift());
        if (r.mtype == Thrift.MessageType.EXCEPTION) {
            //BUG: There is a struct which is not deserialized so we fake
            //this here to create a valid stack.
    ...
    readMessageEnd: function () {
        if (this.rstack.length > 0){
            console.warn("Deserialization Stack is not empty.");
        }
    },
    /** 
...
//            //Transform it onto the stack
//            while (ob.length >= 0) {
//                this.rpush(ob.pop());
//            }
//            //take the real first struct
//            ob = this.rpop();
//        }


        this.rpush({"fid": -1, "fname": "", "ftype": Thrift.Type.STOP});
        for (var fid in ob) {
            var inner = ob[fid];
            for (var ftype  in inner) { //will only find one child
                this.rpush({"fid": parseInt(fid), "fname": "", "ftype": Thrift.Protocol.RType[ftype], "val": inner[ftype]});
            }
        }
    ...
    readFieldBegin: function () {
        ...
        if (r.ftype !== Thrift.Type.STOP) {
            this.rpush(r.val);
            r.val = undefined; //do not give val back
        }
    ...
    readMapBegin: function () {
        ...
        for (var key in vals) {            
            this.rstack.push(vals[key]);
            this.rstack.push(key);
        }
    ...
    readListBegin: function () {
        var lst = this.rpop()
        var r = {};
        r.etype = Thrift.Protocol.RType[lst.shift()];
        r.size = lst.shift();
